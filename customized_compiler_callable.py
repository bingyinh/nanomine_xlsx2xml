...
## Updated to use python 3 on 07/07/2019
...
def sortSequence(myList, myClassName, myXSDtree):
    ...
    CTmap = {'Distribution':'Distribution',# small_list_data;'Distribution'
             ...
             'Extrusion':'ExtrusionType',# temp;prevTemp
...
def sheetPropMech(sheet, DATA_PROP, myXSDtree, jobDir, start_row=0, stop_sign=set()):
    ...
    for row in range(start_row, sheet.nrows):
        ...
        if match(sheet.cell_value(row, 0), "Poisson's ratio"):
            ...
            while len(myRow) < 7:
                myRow.append('') # prevent IndexError
            poiR = addKVU('PoissonsRatio', myRow[1], myRow[2], '', '', '', '', '', poiR, jobDir, myXSDtree)
            if len(poiR) > 0:
                temp.append(poiR)
        # Flexural
            # FlexuralModulus
...
def compiler(jobDir, code_srcDir, xsdDir, templateName):
    ...
    if len(DOI) > 0 and 'ma-SI' not in DOI:
        ...
        citationEle = etree.XML(alldoiDict[DOI]['metadata']).find('.//Citation')
    ...
    # make directory for xml output (easy for debug)
